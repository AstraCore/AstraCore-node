...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'astracore-lib': 'https://github.com/216k155/astracore-lib.git#master',
    'astracore-node': 'https://github.com/216k155/astracore-node.git#master'
  }
};

/**
 * Will create a directory and astra.conf file for Bitcoin.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/astracore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the astracore-node directory and configuration
