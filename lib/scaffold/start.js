...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for astrad is necessary for this upgrade with the "reindex=1" astra.conf option. \n' +
                 'There are changes necessary in both astra.conf and astracore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed astracore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './astracore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in astracore-node services directory
    ...
    if (servicePackage.astracoreNode) {
      serviceModule = service.name + '/' + servicePackage.astracoreNode;
    }
