...
var bitcore = require('astracore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var AstraRPC = require('astrad-rpc');
...
/**
 * Provides a friendly event driven API to astrad in Node.js. Manages starting and
 * stopping astrad as a child process for application support, as well as connecting
 * to multiple astrad processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Astra(options) {
  if (!(this instanceof Astra)) {
    return new Astra(options);
  }
...
util.inherits(Astra, Service);

Astra.dependencies = [];

Astra.DEFAULT_MAX_TXIDS = 1000;
Astra.DEFAULT_MAX_HISTORY = 50;
Astra.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Astra.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Astra.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Astra.DEFAULT_SPAWN_RESTART_TIME = 5000;
Astra.DEFAULT_SPAWN_STOP_TIME = 10000;
Astra.DEFAULT_TRY_ALL_INTERVAL = 1000;
Astra.DEFAULT_REINDEX_INTERVAL = 10000;
Astra.DEFAULT_START_RETRY_INTERVAL = 5000;
Astra.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Astra.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Astra.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Astra.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Astra.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Astra.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Astra.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Astra.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Astra.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Astra.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Astra.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Astra.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Astra.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Astra.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Astra.prototype._initCaches = function() {
...
Astra.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Astra.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Astra.prototype.getPublishEvents = function() {
  return [
    {
...
Astra.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'astrad/' + name, 'total:', this.subscriptions[name].length);
};

Astra.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'astrad/' + name, 'total:', this.subscriptions[name].length);
};


Astra.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'astrad/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Astra.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'astrad/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Astra.prototype.unsubscribeBalanceAll = function(emitter) {
...
Astra.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'astrad/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Astra.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'astrad/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Astra.prototype.unsubscribeAddressAll = function(emitter) {
...
Astra.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Astra.DEFAULT_CONFIG_SETTINGS;
...
Astra.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Astra.prototype._expandRelativeDatadir = function() {
...
Astra.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in astrad config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in astrad config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in astrad config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './astra.conf');

  log.info('Using astra config file:', configPath);
...
Astra.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of astracore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of astracore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of astracore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to astrad from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from astrad. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from astrad. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in astra.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that astrad is undergoing a reindex. ' +
             ...
             'of astracore-node services will start.');
    node._reindex = true;
  }
};

Astra.prototype._resetCaches = function() {
...
Astra.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Astra.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Astra.prototype._initChain = function(callback) {
  var self = this;
...
Astra.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9888
  };
...
Astra.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/astra.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/astra.conf';
    }
  }
  return networkPath;
};

Astra.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Astra.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('astrad/hashblock', message.toString('hex'));
    }
  }

};

Astra.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Astra.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Astra Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Astra.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Astra.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Astra.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('astrad/addressbalance', {
                    address: address,
                    txid: txid,
...
Astra.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('astrad/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Astra.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('astrad/rawtransaction', message.toString('hex'));
    }
...
Astra.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Astra.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Astra.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Astra.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Astra.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Astra Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Astra.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Astra.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Astra.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/astrad.pid';
...
Astra.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting astra process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Astra process unexpectedly exited with code:', code);
        log.warn('Restarting astra child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Astra process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn astrad.'));
      }
...
Astra.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to astrad.'));
    }
 ...
 */
Astra.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Astra configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Astra.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.syncPercentage = function(callback) {
  var self = this;
...
Astra.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, astracore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Astra.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Astra.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Astra.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Astra.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in astracore is most recent to less recent
 ...
 * @param {Function} callback
 */
Astra.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Astra.prototype._getConfirmationsDetail = function(transaction) {
...
Astra.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Astra.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Astra.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A astracoin transaction id
 * @param {Function} callback
 */
Astra.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Astra.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Astra.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Astra.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Astra.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Astra.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Astra.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Astra.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Astracore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Astra.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getInfo = function(callback) {
...
Astra.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('astrad spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('astrad process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of astra addresses to filter
 * @param {Function} callback
 */
Astra.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Astra.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Astra.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Astra.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Astra.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Astra.prototype.getStakingStatus = function(callback) {
    var self = this,
