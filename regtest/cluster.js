...
describe('Bitcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/astrad');
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/astra.conf'),
      rpcuser: 'bitcoin',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/astra.conf'),
      rpcuser: 'bitcoin',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/astra.conf'),
      rpcuser: 'bitcoin',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 astrad daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three astrad daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.astrad.once('tip', function(height) {
