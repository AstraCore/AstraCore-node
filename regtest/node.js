...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.astrad.height === 150) {
            node.services.astrad.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.astrad.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('astrad/hashblock', function(data) {
        bus.unsubscribe('astrad/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.astrad.height;
      node.services.astrad.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.astrad.sendTransaction(tx.serialize(), function(err, hash) {
