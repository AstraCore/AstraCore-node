...
describe('#defaultBaseConfig', function() {
  it('will return expected configuration', function() {
    ...
    info.config.services.should.deep.equal(['astrad', 'web']);
    var astrad = info.config.servicesConfig.astrad;
    astrad.spawn.datadir.should.equal(home + '/.astra');
    astrad.spawn.exec.should.equal(path.resolve(__dirname, '../../bin/astrad'));
  });
  ...
  it('be able to specify a datadir', function() {
    ...
    info.config.servicesConfig.astrad.spawn.datadir.should.equal('./data2');
  });
});
