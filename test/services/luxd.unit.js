...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/astra.conf')));
var AstraService = proxyquire('../../lib/services/astrad', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultAstraConf = fs.readFileSync(path.resolve(__dirname, '../data/default.astra.conf'), 'utf8');

describe('Astra Service', function() {
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var astrad = new AstraService(baseConfig);
			should.exist(astrad);
		});
		it('will create an instance without `new`', function() {
			var astrad = AstraService(baseConfig);
			should.exist(astrad);
		});
		it('will init caches', function() {
			var astrad = new AstraService(baseConfig);

			should.exist(astrad.utxosCache);
			should.exist(astrad.txidsCache);
			should.exist(astrad.balanceCache);
			should.exist(astrad.summaryCache);
			should.exist(astrad.transactionDetailedCache);
			should.exist(astrad.accountInfo);

			should.exist(astrad.transactionCache);
			should.exist(astrad.rawTransactionCache);
			should.exist(astrad.rawJsonTransactionCache);
			should.exist(astrad.blockCache);
			should.exist(astrad.blockJsonCache);
			should.exist(astrad.blockSubsidyCache);
			should.exist(astrad.rawBlockCache);
			should.exist(astrad.blockHeaderCache);
			should.exist(astrad.zmqKnownTransactions);
			should.exist(astrad.zmqKnownBlocks);
			should.exist(astrad.lastTip);
			should.exist(astrad.lastTipTimeout);
			should.equal(astrad.dgpInfoCache, null, 'should be null');
			should.equal(astrad.miningInfoCache, null, 'should be null');
			should.equal(astrad.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(astrad.maxTxids, 1000);
			should.equal(astrad.maxTransactionHistory, 50);
			should.equal(astrad.maxAddressesQuery, 10000);
			should.equal(astrad.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(astrad.spawnRestartTime, 5000);
			should.equal(astrad.spawnStopTime, 10000);

			// try all interval
			should.equal(astrad.tryAllInterval, 1000);
			should.equal(astrad.startRetryInterval, 5000);

			// rpc limits
			should.equal(astrad.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(astrad.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var astrad = new AstraService(baseConfig);
			astrad.nodes.should.deep.equal([]);
			astrad.nodesIndex.should.equal(0);
			astrad.nodes.push({ client: sinon.stub() });
			should.exist(astrad.client);
		});
		it('will set subscriptions', function() {
			var astrad = new AstraService(baseConfig);
			astrad.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var astrad = new AstraService(baseConfig);
			astrad._initDefaults({ transactionConcurrency: 10 });
			astrad.transactionConcurrency.should.equal(10);
			astrad._initDefaults({});
			astrad.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			AstraService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var astrad = new AstraService(baseConfig);
			var methods = astrad.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var astrad = new AstraService(baseConfig);
			var events = astrad.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('astrad/rawtransaction');
			events[0].scope.should.equal(astrad);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('astrad/hashblock');
			events[1].scope.should.equal(astrad);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('astrad/addresstxid');
			events[2].scope.should.equal(astrad);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('astrad/addressbalance');
			events[3].scope.should.equal(astrad);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var astrad = new AstraService(baseConfig);
			astrad.subscribe = sinon.stub();
			astrad.unsubscribe = sinon.stub();
			var events = astrad.getPublishEvents();

			events[0].subscribe('test');
			astrad.subscribe.args[0][0].should.equal('rawtransaction');
			astrad.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			astrad.unsubscribe.args[0][0].should.equal('rawtransaction');
			astrad.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			astrad.subscribe.args[1][0].should.equal('hashblock');
			astrad.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			astrad.unsubscribe.args[1][0].should.equal('hashblock');
			astrad.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var astrad = new AstraService(baseConfig);
			var emitter = {};
			astrad.subscribe('hashblock', emitter);
			astrad.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			astrad.subscribe('rawtransaction', emitter2);
			astrad.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			astrad.subscribe('hashblock', emitter1);
			astrad.subscribe('hashblock', emitter2);
			astrad.subscribe('hashblock', emitter3);
			astrad.subscribe('hashblock', emitter4);
			astrad.subscribe('hashblock', emitter5);
			astrad.subscriptions.hashblock.length.should.equal(5);

			astrad.unsubscribe('hashblock', emitter3);
			astrad.subscriptions.hashblock.length.should.equal(4);
			astrad.subscriptions.hashblock[0].should.equal(emitter1);
			astrad.subscriptions.hashblock[1].should.equal(emitter2);
			astrad.subscriptions.hashblock[2].should.equal(emitter4);
			astrad.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var astrad = new AstraService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			astrad.subscriptions.hashblock = [emitter1];
			astrad.unsubscribe('hashblock', emitter3);
			astrad.subscriptions.hashblock.length.should.equal(1);
			astrad.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			astrad.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(astrad.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var astrad = new AstraService(baseConfig);
			var emitter = new EventEmitter();
			astrad.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var astrad = new AstraService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			astrad.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			astrad.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeAddress(emitter1);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			astrad.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(astrad, 'unsubscribeAddressAll');
			astrad.unsubscribeAddress(emitter1);
			astrad.unsubscribeAddressAll.callCount.should.equal(1);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			astrad.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			astrad.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			astrad.unsubscribeAddressAll(emitter1);
			astrad.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			astrad.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(astrad.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var astrad = new AstraService(baseConfig);
			var config = astrad._getDefaultConfig();
			config.should.equal(defaultAstraConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a astra.conf file', function() {
			var TestAstra = proxyquire('../../lib/services/astrad', {
				fs: {
					readFileSync: readFileSync,
			...
			var astrad = new TestAstra(baseConfig);
			astrad.options.spawn.datadir = '/tmp/.astra';

			sinon.spy(astrad, '_expandRelativeDatadir');
			sinon.spy(astrad, '_checkConfigIndexes');
			sinon.spy(astrad, '_getDefaultConf')
			sinon.spy(astrad, '_getNetworkConfigPath');
			sinon.spy(astrad, '_parseBitcoinConf');

			var node = {};
			astrad._loadSpawnConfiguration(node);

			astrad._expandRelativeDatadir.callCount.should.equal(1);
			astrad._checkConfigIndexes.callCount.should.equal(1);
			astrad._getDefaultConf.callCount.should.equal(1);
			astrad._getNetworkConfigPath.callCount.should.equal(1);
			astrad._parseBitcoinConf.callCount.should.equal(2);

			should.exist(astrad.spawn.config);
			astrad.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: astracore.Networks.testnet,
					configPath: '/tmp/.astracore/astracore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var astrad = new TestAstra(config);
			...
			astrad.options.spawn.datadir = './data';
			var node = {};
			astrad._loadSpawnConfiguration(node);
			...
			astrad.options.spawn.datadir.should.equal('/tmp/.astracore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestAstra = proxyquire('../../lib/services/astrad', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badastra.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var astrad = new TestAstra(baseConfig);
			(function() {
				astrad._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(astracore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the astra.conf file', function() {
			var astrad = new AstraService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				astrad._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the astra.conf file', function() {
			var astrad = new AstraService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the astra.conf file', function() {
			var astrad = new AstraService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the astra.conf file', function() {
			var astrad = new AstraService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the astra.conf file', function() {
			var astrad = new AstraService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				astrad.transactionDetailedCache.set(keys[i], {});
				astrad.utxosCache.set(keys[i], {});
				astrad.accountInfo.set(keys[i], {});
				astrad.txidsCache.set(keys[i], {});
				astrad.balanceCache.set(keys[i], {});
				astrad.summaryCache.set(keys[i], {});
				astrad.blockOverviewCache.set(keys[i], {});
				astrad.blockJsonCache.set(keys[i], {});
			}
			astrad._resetCaches();
			should.equal(astrad.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(astrad.utxosCache.get(keys[0]), undefined);
			should.equal(astrad.accountInfo.get(keys[0]), undefined);
			should.equal(astrad.txidsCache.get(keys[0]), undefined);
			should.equal(astrad.balanceCache.get(keys[0]), undefined);
			should.equal(astrad.summaryCache.get(keys[0]), undefined);
			should.equal(astrad.blockOverviewCache.get(keys[0]), undefined);
			should.equal(astrad.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			astrad._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				astrad.nodes[0].client.getInfo.callCount.should.equal(1);
				astrad.nodes[1].client.getInfo.callCount.should.equal(1);
				astrad.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			astrad._tryAllClients(function(client, next) {
				...
				astrad.nodes[1].client.getInfo.callCount.should.equal(1);
				astrad.nodes[2].client.getInfo.callCount.should.equal(1);
				astrad.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert astrad-rpc error object into JavaScript error', function() {
			var astrad = new AstraService(baseConfig);
			var error = astrad._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			astrad.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			astrad.nodes.push({
				client: {
			...
			astrad._initChain(function() {
				log.info.callCount.should.equal(1);
				astrad.getRawBlock.callCount.should.equal(1);
				astrad.getRawBlock.args[0][0].should.equal('genesishash');
				astrad.height.should.equal(5000);
				astrad.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			astrad._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			astrad.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			astrad._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			astracore.Networks.disableRegtest();
			baseConfig.node.network = astracore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: astracore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var astrad = new AstraService(config);
			astrad._getDefaultConf().rpcport.should.equal(9888);
		});
		it('will get default rpc port for testnet', function() {
			...
			var astrad = new AstraService(config);
			astrad._getDefaultConf().rpcport.should.equal(9777);
		});
		it('will get default rpc port for regtest', function() {
			...
			var astrad = new AstraService(config);
			astrad._getDefaultConf().rpcport.should.equal(9666);
		});
	});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var astrad = new AstraService(config);
			should.equal(astrad._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var astrad = new AstraService(config);
			astrad._getNetworkConfigPath().should.equal('testnet3/astra.conf');
		});
		it('will get default rpc port for regtest', function() {
			astracore.Networks.enableRegtest();
			var config = {
			...
			var astrad = new AstraService(config);
			astrad._getNetworkConfigPath().should.equal('regtest/astra.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var astrad = new AstraService(baseConfig);
			astrad.node.network = astracore.Networks.testnet;
			astrad._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var astrad = new AstraService(baseConfig);
			astrad.node.network = astracore.Networks.testnet;
			astracore.Networks.enableRegtest();
			astrad._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var astrad = new AstraService(baseConfig);
			astrad.node.network = astracore.Networks.livenet;
			astracore.Networks.enableRegtest();
			should.equal(astrad._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			astrad._rapidProtectedUpdateTip = sinon.stub();
			astrad.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			astrad._zmqBlockHandler(node, message);
			astrad._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			astrad._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(astrad, 'emit');
			astrad.on('block', function(block) {
				block.should.equal(message);
				astrad.emit.callCount.should.equal(1);
				astrad._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			astrad._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			astrad._zmqBlockHandler(node, message);
			astrad._rapidProtectedUpdateTip.callCount.should.equal(1);
			astrad._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			astrad._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(astrad, 'emit');
			var emitter = new EventEmitter();
			astrad.subscriptions.hashblock.push(emitter);
			emitter.on('astrad/hashblock', function(blockHash) {
				astrad._rapidProtectedUpdateTip.callCount.should.equal(1);
				astrad.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				astrad._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub();
			sinon.spy(astrad, '_resetCaches');
			astrad.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				astrad._resetCaches.callCount.should.equal(1);
			...
			astrad._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			astrad.on('synced', function() {
				astrad.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			astrad.on('synced', function() {
				astrad.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			astrad.on('error', function(err) {
				log.error.callCount.should.equal(1);
				astrad.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub();
			astrad._resetCaches = sinon.stub();
			astrad.on('tip', function(height) {
				astrad._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				astrad.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using astracore.Transaction', function() {
			var astrad = new AstraService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = astracore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(astracore.Networks.testnet);
			var outputAddress = astracore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = astracore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: astracore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = astrad._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(astracore.Transaction.Output({
				script: astracore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = astrad._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var astrad = new AstraService(baseConfig);
			var tx = astracore.Transaction();
			var address = astracore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(astracore.Transaction.Output({
				script: astracore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(astracore.Transaction.Output({
				script: astracore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			astrad._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			astrad.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('astrad/addresstxid', function(data) {
			...
			astrad._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			astrad.subscriptions.rawtransaction.push(emitter);
			emitter.on('astrad/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			astrad._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(astrad, 'emit');
			astrad.subscriptions.rawtransaction.push(emitter);
			emitter.on('astrad/rawtransaction', function() {
				done();
			});
			var node = {};
			astrad._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._updateTip = sinon.stub();
			astrad._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			astrad.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			astrad._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				astrad._updateTip.callCount.should.equal(11);
				astrad._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			astrad._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				astrad._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			astrad._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				astrad._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			astrad._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				astrad._zmqBlockHandler.callCount.should.equal(0);
				astrad._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var astrad = new AstraService(baseConfig);
			var node = {};
			astrad._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			astrad._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			astrad._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			astrad._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			astrad.on('tip', function(height) {
				...
				astrad.height.should.equal(100);
				done();
			});
			astrad._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestAstraService = proxyquire('../../lib/services/astrad', {
				fs: {
					readFile: readFile
				}
			});
			var astrad = new TestAstraService(baseConfig);
			astrad.spawnStopTime = 1;
			astrad._process = {};
			astrad._process.kill = sinon.stub();
			astrad._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				astrad._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			astrad._process.kill = sinon.stub().throws(error2);
			astrad._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._loadSpawnConfiguration = sinon.stub();
			astrad._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			astrad._spawnChildProcess(function(err) {
				astrad._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var astrad = new AstraService(baseConfig);
			astrad._loadSpawnConfiguration = sinon.stub();
			astrad._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			astrad._spawnChildProcess(function(err) {
				astrad._loadSpawnConfiguration.callCount.should.equal(1);
				astrad._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var astrad = new TestAstraService(config);
			astrad.spawn = {};
			astrad._loadSpawnConfiguration = sinon.stub();
			astrad._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			astrad.node.stopping = true;
			astrad._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			astrad._loadSpawnConfiguration = sinon.stub();
			astrad.spawn = {};
			astrad.spawn.exec = 'testexec';
			astrad.spawn.configPath = 'testdir/astra.conf';
			astrad.spawn.datadir = 'testdir';
			astrad.spawn.config = {};
			astrad.spawn.config.rpcport = 20001;
			astrad.spawn.config.rpcuser = 'astra';
			astrad.spawn.config.rpcpassword = 'password';
			astrad.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			astrad._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			astrad._initZmqSubSocket = sinon.stub();
			astrad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			astrad._checkReindex = sinon.stub().callsArgWith(1, null);
			astrad._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/astra.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				astrad._loadTipFromNode.callCount.should.equal(1);
				astrad._initZmqSubSocket.callCount.should.equal(1);
				should.exist(astrad._initZmqSubSocket.args[0][0].client);
				astrad._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				astrad._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(astrad._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn astrad spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestAstraService = proxyquire('../../lib/services/astrad', {
				fs: {
			...
			astrad._loadSpawnConfiguration = sinon.stub();
			astrad.spawn = {};
			astrad.spawn.exec = 'astrad';
			astrad.spawn.datadir = '/tmp/astra';
			astrad.spawn.configPath = '/tmp/astra/astra.conf';
			astrad.spawn.config = {};
			astrad.spawnRestartTime = 1;
			astrad._loadTipFromNode = sinon.stub().callsArg(1);
			astrad._initZmqSubSocket = sinon.stub();
			astrad._checkReindex = sinon.stub().callsArg(1);
			astrad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			astrad._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(astrad, '_spawnChildProcess');
			astrad._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						astrad._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			astrad._spawnChildProcess(function(err) {
				...
				astrad._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				astrad.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn astrad spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestAstraService = proxyquire('../../lib/services/astrad', {
				fs: {
			...
			astrad._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						astrad._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			astrad.spawn.config.rpcpassword = 'password';
			astrad.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			astrad._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			astrad._spawnChildProcess(function(err) {
				astrad._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			astrad._initZmqSubSocket = sinon.stub();
			astrad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			astrad._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var astrad = new AstraService(config);
			astrad.node.stopping = true;
			astrad.startRetryInterval = 100;
			astrad._loadTipFromNode = sinon.stub();
			astrad._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to astrad/);
				astrad._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			astrad.startRetryInterval = 1;
			var config = {};
			astrad._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				astrad._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			astrad._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			astrad._connectProcess(config, function(err, node) {
				should.not.exist(err);
				astrad._loadTipFromNode.callCount.should.equal(1);
				astrad._initZmqSubSocket.callCount.should.equal(1);
				astrad._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.options = {};
			astrad.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Astra configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			astrad.options = {
				connect: [
					{}
				]
			...
			astrad.start(function(err) {
				astrad._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var astrad = new AstraService(baseConfig);
			var node = {};
			astrad._initChain = sinon.stub().callsArg(0);
			astrad._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			astrad.options = {
				spawn: {}
			};
			astrad.start(function(err) {
				should.not.exist(err);
				astrad.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(astrad, '_spawnChildProcess');
			var nodes = [{}];
			astrad._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			astrad.options = {
				connect: [
					{}
				]
			...
			astrad.start(function(err) {
				should.not.exist(err);
				astrad._initChain.callCount.should.equal(1);
				astrad._connectProcess.callCount.should.equal(1);
				astrad.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			astrad.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				astrad.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			astrad.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			astrad.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			astrad.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			astrad.syncPercentage(function(err) {
				should.exist(err);
				astrad.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			astrad.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				astrad.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var astrad = new AstraService(baseConfig);
			var args = astrad._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var astrad = new AstraService(baseConfig);
			var args = astrad._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_normalizeAddressArg');
			astrad.nodes.push({
				client: {
			...
			var options = {};
			astrad.getAddressBalance(address, options, function(err) {
				astrad._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			astrad.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				astrad.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					astrad._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			astrad.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = astrad._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = astrad._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = astrad._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = astrad._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				astrad._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			astrad.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(astrad, '_getHeightRangeQuery');
			sinon.spy(astrad, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			astrad.getAddressTxids(address, options, function(err) {
				should.exist(err);
				astrad._getHeightRangeQuery.callCount.should.equal(1);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			astrad.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				astrad._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			astrad.getAddressTxids(address, options, function(err, txids) {
				...
				astrad.getAddressTxids(address, options, function(err, txids) {
					...
					astrad._getHeightRangeQuery.callCount.should.equal(2);
					astrad._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			astrad.getAddressTxids(address, options, function(err, txids) {
				...
				astrad.getAddressTxids(address, options, function(err, txids) {
					...
					astrad._getHeightRangeQuery.callCount.should.equal(4);
					astrad._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			astrad.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				astrad.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					astrad.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						astrad._getHeightRangeQuery.callCount.should.equal(3);
						astrad._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var astrad = new AstraService(baseConfig);
			astrad.height = 10;
			var confirmations = astrad._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var astrad = new AstraService(baseConfig);
			var result = {};
			astrad._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var astrad = new AstraService(baseConfig);
			var result = {};
			astrad._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var astrad = new AstraService(baseConfig);
			var result = {};
			astrad._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var astrad = new AstraService(baseConfig);
			var result = {};
			astrad._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(astrad, '_getAddressDetailsForInput');
			sinon.spy(astrad, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = astrad._getAddressDetailsForTransaction(tx, addresses);
			...
			astrad._getAddressDetailsForInput.callCount.should.equal(1);
			astrad._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_getConfirmationsDetail');
			astrad.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			astrad.height = 300;
			var addresses = {};
			astrad._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			astrad._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				astrad.getDetailedTransaction.callCount.should.equal(1);
				astrad._getAddressDetailsForTransaction.callCount.should.equal(1);
				astrad._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var astrad = new AstraService(baseConfig);
			astrad.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			astrad._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				astrad.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from astracore addresses', function() {
			var addresses = [
				astracore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				astracore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var astrad = new AstraService(baseConfig);
			var strings = astrad._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var astrad = new AstraService(baseConfig);
			(function() {
				astrad._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_normalizeAddressArg');
			astrad.maxAddressesQuery = 100;
			astrad.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getAddressStrings');
			astrad.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getAddressStrings');
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			astrad.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getAddressStrings');
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			astrad._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			astrad.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad._getAddressStrings.callCount.should.equal(1);
				astrad.getAddressTxids.callCount.should.equal(1);
				astrad._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getAddressStrings');
			sinon.spy(astrad, 'getAddressTxids');
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			astrad.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getAddressStrings');
			sinon.spy(astrad, '_paginateTxids');
			astrad._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(astrad, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			astrad.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad._getAddressStrings.callCount.should.equal(1);
				astrad.getAddressTxids.callCount.should.equal(1);
				astrad._getAddressDetailedTransaction.callCount.should.equal(2);
				astrad._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			astrad.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			astrad.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			astrad.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			astrad.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			astrad.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			astrad.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			astrad.getAddressSummary(address, options, function(err, summary) {
				astrad._paginateTxids.callCount.should.equal(1);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad.getAddressTxids.callCount.should.equal(1);
				astrad.getAddressBalance.callCount.should.equal(1);
				astrad._paginateTxids.args[0][1].should.equal(0);
				astrad._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			astrad.getAddressSummary(address, options, function(err, summary) {
				...
				astrad.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					astrad._paginateTxids.callCount.should.equal(0);
					astrad._normalizeAddressArg.callCount.should.equal(2);
					astrad.getAddressTxids.callCount.should.equal(1);
					astrad.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			astrad.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				astrad._paginateTxids.callCount.should.equal(1);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				astrad.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			astrad._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			astrad.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(astrad, '_tryAllClients');
			astrad.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			astrad.getRawBlock(blockhash, function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			astrad.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			astrad.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			astrad.getBlock(0, function(err) {
				astrad._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as astracore object from height', function(done) {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			astrad.getBlock(0, function(err, block) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(astracore.Block);
				done();
			});
		});
		it('will getblock as astracore object', function(done) {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			astrad.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			astrad.getBlock(hash, function(err, block) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			astrad.getBlock(0, function(err, block) {
				...
				astrad.getBlock(0, function(err, block) {
					should.not.exist(err);
					astrad._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			astrad.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			astrad.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			astrad.getBlockHeader(10, function(err) {
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			astrad.getBlockHeader(blockhash, function(err) {
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			astrad.getBlockHeader(0, function(err) {
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			astrad.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				astrad._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			astrad.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			astrad._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				astrad._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			astrad._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			astrad._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_maybeGetBlockHash');
			astrad._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			astrad.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			astrad.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				astrad.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					astrad._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			astrad.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			astrad.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = astracore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			astrad.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			astrad.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			astrad.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = astracore.Transaction();
			(function() {
				astrad.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getRawTransaction('txid', function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			astrad.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getTransaction('txid', function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			astrad.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				astrad._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(astracore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			astrad.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				astrad._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			astrad.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			astrad.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			astrad.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.node.getNetworkName = sinon.stub().returns('testnet');
			...
			astrad.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			astrad.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			astrad.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				astrad.getSubsidy.callCount.should.equal(1);
				astrad.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			astrad.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			astrad.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.spawn = {};
			astrad.spawn.process = new EventEmitter();
			astrad.spawn.process.kill = sinon.stub();
			astrad.stop(done);
			astrad.spawn.process.kill.callCount.should.equal(1);
			astrad.spawn.process.kill.args[0][0].should.equal('SIGINT');
			astrad.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			astrad.spawn.process.kill.callCount.should.equal(1);
			astrad.spawn.process.kill.args[0][0].should.equal('SIGINT');
			astrad.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var astrad = new AstraService(baseConfig);
			astrad.shutdownTimeout = 300;
			astrad.spawn = {};
			astrad.spawn.process = new EventEmitter();
			astrad.spawn.process.kill = sinon.stub();
			astrad.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_normalizeAddressArg');
			sinon.spy(astrad, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			astrad.nodes.push({
			...
			astrad.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				astrad._getBalanceFromMempool.callCount.should.equal(1);
				astrad._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			astrad.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			astrad.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			astrad.getJsonBlock(0, function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			astrad.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			astrad.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(2);
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			astrad.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
				astrad._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var astrad = new AstraService(baseConfig);
			var emitter = new EventEmitter();
			astrad.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(astrad.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var astrad = new AstraService(baseConfig);
			var emitter = new EventEmitter();
			astrad.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			astrad.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			astrad.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeBalance(emitter1);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			astrad.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var astrad = new AstraService(baseConfig);
			...
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			astrad.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var astrad = new AstraService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(astrad, 'unsubscribeBalanceAll');
			astrad.unsubscribeBalance(emitter1);
			astrad.unsubscribeBalanceAll.callCount.should.equal(1);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			astrad.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			astrad.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			astrad.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			astrad.unsubscribeBalanceAll(emitter1);
			astrad.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			astrad.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			astrad.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(astrad.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var astrad = new AstraService(baseConfig);
			sinon.spy(astrad, '_notifyBalanceSubscriber');
			astrad._getAddressesFromTransaction = sinon.stub().returns(addresses);
			astrad.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('astrad/addressbalance', function(data) {
				...
				astrad._notifyBalanceSubscriber.callCount.should.equal(1);
				astrad._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			astrad._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('astrad/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				astrad.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			astrad._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			astrad.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			astrad.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			astrad.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getDgpInfo(function(err, result) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getMiningInfo(function(err, result) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingStatus', function() {
		it('will give rpc error', function(done) {
			...
			astrad.getStakingStatus(function(err, result) {
				should.exist(err);
				astrad._tryAllClients.callCount.should.equal(1);
